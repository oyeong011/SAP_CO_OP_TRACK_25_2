...
  IF p_dept IS NOT INITIAL AND s_dept[] IS INITIAL.
...
    SELECT a~zpernr, b~zpname, a~zdepcode, a~zdeprank,

           a~zedate, a~zqflag, a~zqdate,

           b~zgender, b~zaddress AS zaddress,
...
      WHERE ( a~datab = '00000000' OR a~datab <= @s_date-high )

        AND ( a~datbi = '00000000' OR a~datbi >= @s_date-low  ).
...
        AND ( a~datab = '00000000' OR a~datab <= @s_date-high )
...
    DELETE gt_pernr WHERE zqflag = c_x
...
    DELETE gt_pernr WHERE NOT ( zqflag = c_x
...
  SELECT COUNT(*) INTO @lv_all FROM zedt19_102.

  MESSAGE |1) 102##: { lv_all }| TYPE 'I'.



  " 2) #### ## (DATAB # HIGH  AND  DATBI # LOW)

  SELECT COUNT(*) INTO @lv_vld

    FROM zedt19_102

    WHERE datab <= @s_date-high

      AND datbi >= @s_date-low.

  MESSAGE |2) ####: { lv_vld }| TYPE 'I'.



  " 3) ### ## (#### ## #)

  SELECT COUNT(*) INTO @lv_flag_sp FROM zedt19_102

    WHERE datab <= @s_date-high AND datbi >= @s_date-low AND zqflag = @space.

  SELECT COUNT(*) INTO @lv_flag_x  FROM zedt19_102

    WHERE datab <= @s_date-high AND datbi >= @s_date-low AND zqflag = 'X'.

  SELECT COUNT(*) INTO @lv_flag_y  FROM zedt19_102

    WHERE datab <= @s_date-high AND datbi >= @s_date-low AND zqflag = 'Y'.

  MESSAGE |3) ###(#/X/Y): { lv_flag_sp } / { lv_flag_x } / { lv_flag_y }| TYPE 'I'.



  " 4) R_FLAG ## ## (build_flag_range # ##)

  SELECT COUNT(*) INTO @lv_flag_in
...
      AND zqflag IN @r_flag.

  MESSAGE |4) R_FLAG##: { lv_flag_in }| TYPE 'I'.



  " 5) JOIN ## ## (##### ##)

  SELECT COUNT(*) INTO @lv_j103

    FROM zedt19_102 AS a

    INNER JOIN zedt19_103 AS b ON a~zpernr = b~zpernr

    WHERE a~datab <= @s_date-high AND a~datbi >= @s_date-low.



  SELECT COUNT(*) INTO @lv_j106
...
  MESSAGE |5) JOIN(#103/#106/##): { lv_j103 } / { lv_j106 } / { lv_jboth }| TYPE 'I'.
