
*&---------------------------------------------------------------------*

*& Report ZEDR02_018

*&---------------------------------------------------------------------*

*&

*&---------------------------------------------------------------------*




REPORT ZEDR02_018.



"20250925 SUBROUTINE ##



DATA : BEGIN OF GS_STUDENT.

  include structure
ZEDT02_001
.

DATA: END OF GS_STUDENT.

DATA : GT_STUDENT LIKE TABLE OF GS_STUDENT.



DATA: GV_ZCODE LIKE ZEDT02_001-ZCODE.

DATA: GV_ZKNAME LIKE ZEDT02_001-ZKNAME.



"GV_ZC0DE = 'SSU-02'.

GS_STUDENT-ZCODE = 'SSU-02'.



"PERFORM GET_DATA USING GV_ZCODE GV_ZKNAME. "Actual Parameter

"PERFORM GET_DATA USING GV_ZCODE CHANGING GV_ZKNAME. "USING: #### ##(### # # #) CHANGING: #### #### ##(### # # #)<-### ##

"PERFORM GET_DATA USING GS_STUDENT.

"PERFORM GET_DATA USING GS_STUDENT GT_STUDENT.




*PERFORM GET_DATA USING GT_STUDENT.




"PERFORM GET_DATA TABLES GT_STUDENT.

PERFORM GET_DATA CHANGING GT_STUDENT.






*WRITE : / GV_ZCODE.

*WRITE : / GV_ZKNAME.

*WRITE: / GS_STUDENT-ZCODE.

*WRITE: / GS_STUDENT-ZKNAME.




LOOP AT GT_STUDENT INTO GS_STUDENT.

  WRITE: / GS_STUDENT-ZCODE, GS_STUDENT-ZKNAME.

ENDLOOP.






*&---------------------------------------------------------------------*

*&      Form  GET_DATA

*&---------------------------------------------------------------------*

*       text

*----------------------------------------------------------------------*

*  -->  p1        text

*  <--  p2        text

*----------------------------------------------------------------------*




"FORM GET_DATA USING P_ZCODE P_ZKNAME. "USING Form Parameter(## #####) PASS BY REFERENCE

"FORM GET_DATA USING VALUE(P_ZCODE) P_ZKNAME. "USING VALUE Actual Parameter ## Subroutine### ## ## ## ## ##->PASS BY VALUE

"FORM GET_DATA USING P_ZCODE TYPE C "## ### ## ###->### #### ## + ###### #####, ##, ### ## ### ####

"      CHANGING P_ZKNAME TYPE C. "CHANGING

"FORM GET_DATA USING P_ZCODE CHANGING VALUE(P_ZKNAME). "CHANGING VALUE

"FORM GET_DATA USING PS_STUDENT STRUCTURE GS_STUDENT. "### ## ### ##




*FORM GET_DATA USING PS_STUDENT STRUCTURE GS_STUDENT "STRUCTURE ## LIKE #### ##

*      PT_STUDENT LIKE GT_STUDENT.

*  "WRITE : 'PERFORM TEST'.

*  "DATA : LV_ZCODE LIKE ZEDT02_001-ZCODE. "#### # #### ##

*  "LV_ZCODE = 'SSU-01'.

*

*  SELECT SINGLE ZKNAME FROM ZEDT02_001

*    INTO P_ZKNAME

*    WHERE ZCODE = P_ZCODE.

*

*  P_ZKNAME = 'TEST'.

*  PS_STUDENT-ZCODE = 'SSU-01'.

*

*  SELECT SINGLE ZKNAME FROM ZEDT02_001

*    INTO PS_STUDENT-ZKNAME

*    WHERE ZCODE = GS_STUDENT-ZCODE.

*  APPEND PS_STUDENT TO PT_STUDENT.

*

*  PS_STUDENT-ZCODE = 'SSU-02'.

*

*  SELECT SINGLE ZKNAME FROM ZEDT02_001

*    INTO PS_STUDENT-ZKNAME

*    WHERE ZCODE = PS_STUDENT-ZCODE.

*  APPEND PS_STUDENT TO PT_STUDENT.

*

*ENDFORM.






"## #### ## ### ## ####(## ### ##### ##)->###, #### ##




*FORM GET_DATA USING PT_STUDENT TYPE STANDARD TABLE.




"FORM GET_DATA TABLES PT_STUDENT LIKE GT_STUDENT. "### # ##

FORM GET_DATA CHANGING PT_STUDENT LIKE GT_STUDENT.

  DATA: LS_STUDENT LIKE GS_STUDENT.



  LS_STUDENT-ZCODE = 'SSU-01'.



  SELECT SINGLE ZKNAME FROM ZEDT02_001

    INTO LS_STUDENT-ZKNAME

    WHERE ZCODE = LS_STUDENT-ZCODE.

    APPEND LS_STUDENT TO PT_STUDENT.



  LS_STUDENT-ZCODE = 'SSU-02'.



  SELECT SINGLE ZKNAME FROM ZEDT02_001

    INTO LS_STUDENT-ZKNAME

    WHERE ZCODE = LS_STUDENT-ZCODE.

    APPEND LS_STUDENT TO PT_STUDENT.

ENDFORM.